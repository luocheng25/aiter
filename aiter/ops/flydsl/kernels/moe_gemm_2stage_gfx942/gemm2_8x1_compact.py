...
def _build_moe_gemm2_8x1_compact(
    ...
    assert K in (
        ...
        512,
        640,
    ), "8x1_compact仅支持K=192/256/320/384/512/640"
...
def build_task_table(experts, *, cu_count=None, min_tail_utilization=0.6):
    ...
    cu_count, numerator, denominator, _ = _balance_parameters(
        cu_count, min_tail_utilization
    )
    return _build_task_table_cached(
        get_device_cache_key(), experts, cu_count, numerator, denominator
    )


@functools.cache
def _build_task_table_cached(
    device_cache_key, experts, cu_count, threshold_numerator, threshold_denominator
):
...
def prepare_tasks(
    ...
    *,
    cu_count=None,
    min_tail_utilization=0.6,
):
    """Tensor便利入口；任务计数始终保留在device，不用于CPU分支或grid决策。"""
    assert (
        sorted_experts.device.type == "cuda"
        and valid_rows.device == sorted_experts.device
    )
    if cu_count is None:
        cu_count = device_cu_count(sorted_experts.device)
    if buffers is None:
        buffers = allocate_task_buffers(
            sorted_experts,
            experts,
            cu_count=cu_count,
            min_tail_utilization=min_tail_utilization,
        )
    required_full, required_tail = task_capacities(
    ...
    assert (
        buffers[0].ndim == buffers[1].ndim == 2
        and buffers[0].shape[1] == buffers[1].shape[1] == 2
    )
    assert (
        buffers[0].shape[0] >= required_full and buffers[1].shape[0] >= required_tail
    ), "任务workspace容量不足，需按新的CU拆分上限分配"
    assert buffers[2].numel() >= 2 and all(
        t.dtype == torch.int32 and t.device == sorted_experts.device for t in buffers
    )
    with torch.cuda.device(sorted_experts.device):
        build_task_table(
            experts, cu_count=cu_count, min_tail_utilization=min_tail_utilization
        )(
            torch_tensor_to_pointer(sorted_experts),
            torch_tensor_to_pointer(valid_rows),
            *(torch_tensor_to_pointer(value) for value in buffers),
