...
def _build_moe_gemm2_8x1_compact(
    ...
    common = {
        "N": N, "K": K, "weight_dtype": weight_dtype,
        "weight_quant_type": weight_quant_type, "act_quant_type": act_quant_type,
        "TOPK": TOPK, "stage": stage, "alg": alg, "E": E,
        "USE_ATOMIC_WRITE": USE_ATOMIC_WRITE, "tile_k": tile_k,
    ...
    def launch_compact(
        ...
        if const_expr(N % 256 == 0):
            tail(
                p_input, p_weight, p_output, p_sorted_ids, p_sorted_weights,
                p_tail_tasks, p_counts + 1, p_w_scale, p_a_scale, M, tail_capacity,
                stream,
            )
        else:
            tail(
                p_input, p_weight, p_output, p_sorted_ids, p_sorted_weights,
                p_tail_tasks, p_counts + 1, p_w_scale, p_a_scale, M, tail_capacity,
                fx.Float32(1.0), fx.Float32(1.0), fx.Float32(1.0), fx.Float32(1.0),
                fx.Float32(float("inf")), stream,
            )
